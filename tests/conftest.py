"""
Pytest configuration and shared fixtures for SNMP protocol tests.
"""
...
def pytest_collection_modifyitems(config, items):
    """Modify test collection to add default markers if missing."""
    for item in items:
        # Add default bundle marker if not present
        if not any(mark.name == 'bundle' for mark in item.iter_markers()):
            item.add_marker(pytest.mark.bundle(1))
        
        # Add default points marker if not present
        if not any(mark.name == 'points' for mark in item.iter_markers()):
            item.add_marker(pytest.mark.points(1))


# JSON report hook - conditionally loaded
# The pytest_json_runtest_metadata hook is specific to pytest-json-report plugin
# To avoid conflicts, we only define it when the plugin is actively being used
# See conftest_json_report.py for the hook implementation

def pytest_addoption(parser):
    """Add custom command line options."""
    # This is called early in pytest's startup
    pass

def pytest_configure(config):
    """Configure pytest - register markers and conditionally load JSON report hooks."""
    # Register custom markers (moved from bottom)
